...
function TrackShipment() {
  ...
  const handleSubmit = useCallback(async (id: string) => {
    ...
    try {
      router.replace(`/tracking?labelId=${id}`); // Update URL with labelId

      const response = await axios.get(`/api/shipengine/tracking/${id}`);
  ...
  }, [queryLabelId, handleSubmit]); // Dependencies
